...
A git-host capability (`github-ops` or `gitlab-ops`) as the tag-source adapter. `version-drift-tracker` also needs a `service-map.md` you author — it can't run without one. Skip this capability entirely if you only maintain a single service.
